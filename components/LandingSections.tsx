...
const deep = "#09070f";
const surface = "#110f1d";
const text = "#f3efff";
const muted = "#a89fc6";
const pillText = "#140f25";
const cardBg = "rgba(15,10,28,.6)";
...
export function LandingSections() {
  ...
  return (
    <div style={{ position: "fixed", top: 68, left: 0, right: 0, bottom: 0, overflow: "hidden", background: deep }}>
      <style>{`
      ...
      <div data-slide="1" style={{ ...slideBase, transform: tf(1), background: "linear-gradient(180deg,#110f1d 0%,#0b0915 100%)" }}>
        <div style={{ maxWidth: 1080, margin: "auto", padding: "60px 28px", width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            <h2 style={{ fontFamily: serif, fontSize: 36, fontWeight: 600, color: text, margin: 0 }}>Try Our Demo!</h2>
            <p style={{ fontSize: 14.5, color: muted, marginTop: 10 }}>Every module includes a demo. Click below to demo.</p>
          </div>
          <div className="stack-m" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
            {modules.map((m) => (
              <Link key={m.name} href={m.demoHref} style={{ background: cardBg, border: `1px solid ${accentBorder}`, borderRadius: 16, padding: 26, display: "flex", flexDirection: "column", cursor: "pointer", transition: "border-color .18s, box-shadow .18s, transform .18s", color: "inherit" }}
                ...
                onMouseLeave={(e) => { const el = e.currentTarget; el.style.borderColor = accentBorder; el.style.boxShadow = "none"; el.style.transform = "none"; }}>
                ...
                <div style={{ fontSize: 18, fontWeight: 600, color: text, marginBottom: 7 }}>{m.name}</div>
                <div style={{ fontSize: 13.5, color: muted, lineHeight: 1.6, flex: 1 }}>{m.desc}</div>
                <div style={{ marginTop: 18, paddingTop: 16, borderTop: `1px solid ${accentBorder}` }}>
      ...
      <div data-slide="2" style={{ ...slideBase, transform: tf(2), background: accentPanel }}>
        <div style={{ maxWidth: 1080, margin: "auto", padding: "44px 28px", color: text, width: "100%" }}>
          ...
          <div className="stack-m sec-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", maxWidth: 880, margin: "0 auto", border: "1px solid rgba(255,255,255,.14)", borderRadius: 16, overflow: "hidden" }}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                {neverStored.map((t) => (
                  <div key={t.title} style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <span style={{ color: "#fb7185", fontSize: 14, marginTop: 1 }}>x</span>
                    <div>
            ...
            <div style={{ padding: "28px 30px", background: "rgba(155,107,255,.07)" }}>
      ...
      <div data-slide="3" style={{ ...slideBase, transform: tf(3), background: "linear-gradient(180deg,#0b0915 0%,#110f1d 100%)" }}>
        <div style={{ maxWidth: 1080, margin: "auto", padding: "60px 28px", width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            {sectionEyebrow("How it works")}
            <h2 style={{ fontFamily: serif, fontSize: 36, fontWeight: 600, color: text, margin: 0 }}>From documents to roadmap</h2>
          </div>
          <div className="stack-m" style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 16 }}>
            {steps.map((st) => (
              <div key={st.n} style={{ textAlign: "center", padding: "0 8px" }}>
                <div style={{ width: 42, height: 42, borderRadius: "50%", background: `linear-gradient(135deg, ${accentLight}, ${accent})`, color: pillText, display: "flex", alignItems: "center", justifyContent: "center", fontFamily: serif, fontSize: 18, fontWeight: 700, margin: "0 auto 14px" }}>{st.n}</div>
                <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 5 }}>{st.title}</div>
                <div style={{ fontSize: 12.5, color: muted, lineHeight: 1.55 }}>{st.body}</div>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* SLIDE 4 - PRICING */}
      <div data-slide="4" style={{ ...slideBase, transform: tf(4), background: surface }}>
        <div style={{ maxWidth: 1080, margin: "auto", padding: "60px 28px", width: "100%" }}>
          <div style={{ textAlign: "center", marginBottom: 44 }}>
            {sectionEyebrow("Pricing")}
            <h2 style={{ fontFamily: serif, fontSize: 36, fontWeight: 600, color: text, margin: 0 }}>Pay for what you use</h2>
            <p style={{ fontSize: 15, color: muted, marginTop: 10, maxWidth: 540, marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
              Demo every module free with sample data. Activate a module to run it on your own documents.
            </p>
          </div>
          <div className="stack-m" style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 18 }}>
            {modules.map((m) => (
              <div key={m.name} style={{ background: cardBg, border: `1px solid ${accentBorder}`, borderRadius: 16, padding: 28, textAlign: "center" }}>
                <div style={{ fontSize: 15, fontWeight: 600, color: text, marginBottom: 12 }}>{m.name}</div>
                ...
                <Link href="/signin" style={{ display: "block", fontSize: 13.5, color: pillText, fontWeight: 800, padding: 11, background: `linear-gradient(135deg, ${accentLight}, ${accent})`, borderRadius: 999 }}>Get started</Link>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* SLIDE 5 - CTA + FOOTER */}
      <div data-slide="5" style={{ ...slideBase, transform: tf(5), flexDirection: "column", background: "linear-gradient(160deg,#060509 0%,#151022 60%,#09070f 100%)" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "80px 28px", textAlign: "center", flex: 1, display: "flex", flexDirection: "column", justifyContent: "center" }}>
          <h2 style={{ fontFamily: serif, fontSize: 40, fontWeight: 600, color: text, lineHeight: 1.15, maxWidth: 680, margin: "0 auto 18px" }}>Run your first assessment today</h2>
          <p style={{ fontSize: 16, color: muted, maxWidth: 540, margin: "0 auto 30px", lineHeight: 1.6 }}>
            Sign in with your Google, Microsoft, or GitHub account. Try any module in demo mode before you pay.
          </p>
          <div>
            <Link href="/signin" style={{ display: "inline-block", fontSize: 16, color: pillText, fontWeight: 800, padding: "15px 30px", background: `linear-gradient(135deg, ${accentLight}, ${accent})`, borderRadius: 999 }}>Sign in to get started</Link>
          </div>
        </div>
