...
export async function POST(_req: Request, { params }: { params: { id: string } }) {
  ...
  await prisma.usageLedger.create({ data: { accountId: guard.session.user.accountId, kind: "sra_findings", status: isEffectiveAdmin(guard.session) ? "admin_comped" : "succeeded", inputTokens: usage.inputTokens, outputTokens: usage.outputTokens } }).catch(() => undefined);
