...
export default async function AppLauncher() {
  ...
  return <main><Nav/><section className="wrap"><div className="mono">Module launcher</div><h1 style={{fontFamily:"EB Garamond",fontSize:46,margin:"8px 0 24px"}}>Compliance suite</h1><div className="grid">{modules.map((m)=><div className="card" style={{display:"flex",flexDirection:"column",minHeight:260}} key={m.key}><span className={isAdmin || m.credits>0?"badge":"badge locked"}>{isAdmin ? "Admin comped" : m.credits>0?`${m.credits} active credit${m.credits===1?"":"s"}`:"Locked"}</span><h2 style={{marginTop:18}}>{m.title}</h2><p className="muted">{m.body}</p><div style={{marginTop:"auto"}}><div className="mono" style={{margin:"0 0 18px"}}>{m.price}</div><div style={{display:"flex",gap:10,flexWrap:"wrap"}}>{isAdmin || m.credits>0?<Link className="btn" href={m.href}>Launch</Link>:<CheckoutButton module={m.key}>Purchase</CheckoutButton>}<Link className="btn secondary" href={`${m.href}?demo=1`}>Try demo</Link></div></div></div>)}</div></section></main>;
}
