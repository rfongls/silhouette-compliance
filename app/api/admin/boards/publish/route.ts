...
export async function POST(req: Request) {
  ...
  const { id, reviewConfirmed } = await req.json();
  ...
  await prisma.$transaction([
    ...
    prisma.controlBoard.update({ where: { id: draft.id }, data: { status: "PUBLISHED", publishedAt: new Date(), reviewedBy: guard.session.user.email || guard.session.user.id, reviewedAt: new Date(), reviewNotes: "Approved validated control-board draft as the active scoring base." } })
  ]);
