...
async function setAdminViewMode(formData: FormData) {
  ...
  redirect(mode === "customer" ? "/app" : "/admin");
}

export async function Nav({ publicOnly = false }: { publicOnly?: boolean } = {}) {
  const session = publicOnly ? null : await (await import("@/auth")).auth();
  const isAdmin = isAdminSession(session);
  const effectiveRole = getEffectiveRole(session);
  const viewingAsCustomer = isAdmin && effectiveRole === "customer";
  return <nav className="nav"><Link href="/" className="brand">Silhouette<span>.</span></Link><div style={{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}}>{session?.user ? <><Link className="btn secondary" href="/app">App</Link><Link className="btn secondary" href="/app/profile#reports">My reports</Link><Link className="btn secondary" href="/app/profile">Account</Link>{isAdmin && !viewingAsCustomer ? <Link className="btn secondary" href="/admin">Admin</Link> : null}{isAdmin ? <form action={setAdminViewMode}><input type="hidden" name="mode" value={viewingAsCustomer ? "admin" : "customer"} /><button className="btn ghost" title={viewingAsCustomer ? "Return to admin view" : "Preview the customer experience"}>{viewingAsCustomer ? "Admin view" : "Customer view"}</button></form> : null}<form action={async()=>{"use server"; const { signOut } = await import("@/auth"); await signOut({ redirectTo: "/" });}}><button className="btn ghost">Sign out</button></form></> : <Link className="btn secondary" href="/signin">Sign in</Link>}</div></nav>;
}
