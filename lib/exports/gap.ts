...
export function buildGapReport(result: any, opts?: { auditor?: string; title?: string }) {
  ...
  @page{size:letter;margin:.55in}*{box-sizing:border-box}body{font-family:Arial,sans-serif;color:#111;margin:0;background:#fff}.page{min-height:10in;page-break-after:always;padding:.45in}.cover{background:#1f2c26;color:#fff;display:flex;flex-direction:column;justify-content:center}.brand{font-size:12px;letter-spacing:.24em;text-transform:uppercase;color:#9ec5b4}.h1{font-family:Georgia,serif;font-size:42px;line-height:1.08;margin:18px 0}.rule{height:3px;width:4.5in;background:#2d6a4f;margin:18px 0}.muted{color:#6b7280}.cover .muted{color:#bcd0c6}.score{width:104px;height:104px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:10px solid ${color};font-size:34px;font-weight:700;color:${color};margin-right:24px}h2{font-family:Georgia,serif;font-size:24px;margin:0 0 16px;color:#1f4e3b}table{width:100%;border-collapse:collapse;font-size:11px}th{text-align:left;color:#666;text-transform:uppercase;letter-spacing:.08em;border-bottom:2px solid #111;padding:7px}td{border-bottom:1px solid #e5e7eb;padding:8px;vertical-align:top}.phase{border-top:4px solid #2d6a4f;padding:14px;border:1px solid #e5e7eb;margin-bottom:10px}.phase h3{font-family:Georgia,serif;margin:0 0 8px}.phase span{font:12px Arial;color:#777}.footer{position:fixed;bottom:.25in;left:.55in;right:.55in;border-top:1px solid #ddd;padding-top:6px;font-size:9px;color:#888;display:flex;justify-content:space-between}@media print{button{display:none}}
...
export function buildGapDeck(result: any) {
  ...
  return noEmDash(`<!doctype html><html><head><meta charset="utf-8"/><title>${esc(r.organization_name)} Deck</title><style>body{margin:0;font-family:Arial,sans-serif}.slide{min-height:100vh;page-break-after:always;padding:8vh 9vw;display:flex;flex-direction:column;justify-content:center}.dark{background:#1f2c26;color:#fff}.k{letter-spacing:.24em;text-transform:uppercase;color:#2d6a4f;font-size:12px}.title{font-family:Georgia,serif;font-size:54px;line-height:1.05}.score{font-size:110px;font-weight:700;color:${scoreColor(Number(r.compliance_score || 0))}}li{font-size:22px;margin:14px 0;color:#333}.dark li{color:#bcd0c6}</style></head><body><section class="slide dark"><div class="k">Silhouette LLC</div><div class="title">${esc(r.organization_name)}</div><p>Incident Response Plan Gap Analysis</p></section><section class="slide"><div class="k">Executive Summary</div><div class="score">${esc(r.compliance_score)}</div><h1>${esc(r.overall_posture)}</h1><p>${esc(r.posture_summary)}</p></section><section class="slide"><div class="k">Key Findings</div><ul>${findings.map((f: any) => `<li><b>${esc(f.risk_level)}</b> - ${esc(f.finding)}</li>`).join("")}</ul></section><section class="slide dark"><div class="k">Next Steps</div><div class="title">Remediate, test, reassess.</div></section></body></html>`);
}
